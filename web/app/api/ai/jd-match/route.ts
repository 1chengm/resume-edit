...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!resumeContent || !jdText) return NextResponse.json({ error: 'Missing resumeContent or jdText' }, { status: 400 })
    ...
    const { object } = await generateObject({
      model,
      schema: JDMatchSchema as unknown as z.ZodTypeAny,
      system: prompt,
      prompt: JSON.stringify({ resume: sanitized, jd: jdText })
    })
    ...
    if (body.resumeId) {
      ...
      const hash = crypto.createHash('sha256').update(JSON.stringify({ resume: sanitized, jd: jdText })).digest('hex')
